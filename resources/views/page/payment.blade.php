                ...
                <div class="list-voucher">
                    @foreach ($vouchers as $voucher)
                        <div
                            ...
                            <img src="{{ asset('public/frontend/admin/images/logo.png') }}" alt="Voucher Image"
                                width="90" height="90">
                            ...
                            <div class="checkbox">
                                <input type="radio" name="voucher" value="{{ $voucher->voucher_id }}"
                                    class="voucher-radio" data-discount-value="{{ $voucher->discount_value }}">
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateTotal() {
                ...
                var discountValue = selectedVoucher.data('discount-value') || 0; // Lấy giá trị giảm giá từ voucher
                var total = {{ $total }}; // Giá trị tổng hiện tại từ server

                // Tính toán tổng mới sau khi áp dụng giảm giá
                var newTotal = total - (total * (discountValue / 100));

                // Cập nhật tổng tiền hiển thị trên trang
                $('#total span').text(newTotal.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',') + ' VND');

                // Gửi giá trị tổng về server để cập nhật session
                updateSessionTotal(newTotal);
            }

            // Hàm gửi giá trị tổng về server để cập nhật session
            function updateSessionTotal(newTotal) {
                $.ajax({
                    url: '/apply-voucher', // URL cho yêu cầu API
                    type: 'POST',
                    data: {
                        total: newTotal,
                        voucher_id: $('input[name="voucher"]:checked').val(), // Gửi ID của voucher
