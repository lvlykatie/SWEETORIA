            ...
            @foreach($edit_voucher as $key => $voucher)
                <form class="p-10 bg-white rounded shadow-xl" role="form" action="{{URL::to('admin/vouchers/update/'.$voucher->voucher_id)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mt-2 relative">
                        ...
                        <input type="number" class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="minimum_order_value" value="{{$voucher->minimum_order_value }}" name="max_usage" required="" placeholder="Enter voucher's max usage" aria-label="Max usage"></input>
                    </div>
                    <div class="">
